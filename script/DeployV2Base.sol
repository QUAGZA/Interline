...
/// @notice Shared V2 fixture deploy for Anvil (31337) and Base Sepolia (84532).
...
abstract contract DeployV2Base is Script {
    ...
    uint16 internal constant MAX_LTV_BPS = 7000;
    uint16 internal constant LIQ_THRESHOLD_BPS = 8000;
        ...
        returns (DeployedV2 memory d)
    {
        ...
        d.factory = new MarketFactory(curator, address(d.escrow));
        d.vaultFactory = new BorrowerVaultFactory(address(d.venue), address(d.router), address(d.mweth));
        ...
        ILendingMarket.Init memory restricted = wallet;
        restricted.deliveryMode = ILendingMarket.DeliveryMode.Restricted;
        restricted.defaultPositionCap = RESTRICTED_CAP;
        restricted.vaultFactory = address(d.vaultFactory);
